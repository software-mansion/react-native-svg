...
{
    [self clearPath];
    for (__kindof RNSVGNodeFabric *node in self.subviews) {
...
{
    RNSVGNodeFabric* node = self;
    while (node != nil) {
        RNSVGPlatformView* parent = [node superview];

        if (![parent isKindOfClass:[RNSVGNodeFabric class]]) {
            return;
        }
        node = (RNSVGNodeFabric*)parent;
        if (!node.path) {
            return;
        }
        [node clearPath];
...
- (RNSVGGroup *)textRoot
{
    ...
    while (node != nil) {
        ...
        if (![node isKindOfClass:[RNSVGNodeFabric class]]) {
            node = nil;
        } else {
            node = (RNSVGNodeFabric*)parent;
        }
...
{
    RNSVGNodeFabric* parent = (RNSVGGroup*)[self superview];
...
- (RNSVGSvgView *)svgView
{
    ...
    if ([parent class] == [RNSVGSvgView class]) {
        _svgView = parent;
    } else if ([parent isKindOfClass:[RNSVGNodeFabric class]]) {
        _svgView = ((RNSVGNodeFabric *)parent).svgView;
    } else {
