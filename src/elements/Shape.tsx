...
export class SVGMatrix implements SVGMatrix {
  ...
  rotate(angle : number) : SVGMatrix {
    ...
    return new SVGMatrix(
        multiply_matrices(
            this, {a : cos, b : sin, c : -sin, d : cos, e : 0, f : 0}), );
  }
  rotateFromVector(x : number, y : number) : SVGMatrix {
...
export const ownerSVGElement = {createSVGPoint() :
                                    SVGPoint{return new SVGPoint();
}
...
export default class Shape<P> extends Component<P> {
  ...
  refMethod
      : (instance
         : (Shape<P> &NativeMethods) |
             null) = > void = (instance
                               : (Shape<P> & NativeMethods) | null, ) = > {
    this.root = instance;
  };
  setNativeProps = (
      props
  ...
  getBBox = (options ?: SVGBoundingBoxOptions) : SVGRect = > {
    ...
    return RNSVGRenderableManager.getBBox(
        handle,
        {
            fill,
            stroke,
