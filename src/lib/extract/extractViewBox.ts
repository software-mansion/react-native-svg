...
export const meetOrSliceTypes: {[meetOrSlice: string]: number;} = {
  meet : 0,
  slice : 1,
  none : 2,
};
...
export default function extractViewBox(
    props: {viewBox?: string|NumberProp[]; preserveAspectRatio?: string;}) {
  ...
  const modes =
      preserveAspectRatio ? preserveAspectRatio.trim().split(spacesRegExp) : [];
  const align = modes[0];
