...
const TestingView = () => {
  ...
  useEffect(() => {
    wsClient.onopen = () => {
      //todo: use proper arch here
      wsClient.send(JSON.stringify({os: Platform.OS, version: Platform.Version, arch: "paper"}))
    }
    //todo: find out why there is 1005 close when closing WS on Jest side
    wsClient.onerror = (err) => {
      console.log(err)
      if (!err) { // when gracefully stopping WS, the err is null however the callback is still being called for some reason
        return;
      }
      console.error(`Error while connecting to E2E WebSocket server at ${wsUri}: ${err.message}. Reopen this tab to retry.`)
    }
